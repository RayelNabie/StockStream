...
export const assignBarcode = async (warehouseNumber) => {
  try {
    // ✅ Stap 1: Controleer of `warehouseNumber` een geldig getal is
    if (!Number.isInteger(warehouseNumber) || warehouseNumber < 0) {
      error("[Service] Ongeldige warehouseNumber ontvangen", { warehouseNumber });
      return {
        status: 400,
        error: "Warehouse number is invalid. It must be a positive integer.",
      };
    }
    ...
    return {
      status: 500,
      error: "Interne serverfout bij genereren van barcode.",
    };
  }
};